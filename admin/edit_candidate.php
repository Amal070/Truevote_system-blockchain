...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // File upload paths (keep old if no new upload)
    ...
    $stmt = $pdo->prepare("UPDATE candidates 
        SET election_id=?, name=?, gender=?, party_name=?, symbol=?, age=?, photo=?, manifesto=?
        WHERE id=?");
    $stmt->execute([
        $election_id,
        $name,
    ...
    header("Location: view_candidates.php?success=Candidate updated successfully");
    exit;
}
?>
<!DOCTYPE html>
...
<head>
  ...
  <style>
    body { background-color: #0f172a; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; color: #f8fafc; }
    h3 { font-weight: 700; text-align: center; margin-bottom: 1.5rem; color: #e2e8f0; }
    .card { background-color: #1e293b; border-radius: 15px; border: none; box-shadow: 0 4px 15px rgba(0,0,0,0.3); padding: 2rem; max-width: 700px; margin: 2rem auto; width: 95%; }
    .form-label { font-weight: 500; color: #e2e8f0; }
    .form-control, .form-select, textarea { background-color: #0f172a; border: 1px solid #334155; color: #f8fafc; border-radius: 10px; }
    .form-control:focus, .form-select:focus, textarea:focus { border-color: #6366f1; box-shadow: none; }
    .btn-primary { background-color: #10b981; border: none; font-weight: 600; border-radius: 10px; padding: 0.75rem; }
    .btn-primary:hover { background-color: #059669; }
    .upload-box { border: 2px dashed #6366f1; border-radius: 12px; padding: 25px; text-align: center; cursor: pointer; transition: 0.3s; background: #0f172a; color: #e2e8f0; }
    .upload-box:hover { background: #1e293b; border-color: #4f46e5; }
    .upload-box input {display:none;}
    .upload-preview { margin-top: 15px; max-height: 150px; }
    .upload-preview img { max-height: 150px; border-radius: 10px; object-fit: cover; box-shadow: 0 2px 6px rgba(0,0,0,0.3); }
  </style>
</head>
<body>
...
<div class="card">
  ...
  <form method="POST" enctype="multipart/form-data">
    <input type="hidden" name="election_id" value="<?= htmlspecialchars($candidate['election_id']) ?>">
    ...
    <div class="mb-3">
      ...
      <input type="text" name="party_name" class="form-control" value="<?= htmlspecialchars($candidate['party_name']) ?>">
    </div>

    <div class="mb-3">
      <label class="form-label">Age</label>
      <input type="number" name="age" class="form-control" min="18" max="120" value="<?= htmlspecialchars($candidate['age']) ?>">
    </div>

    <div class="mb-3">
      <label class="form-label">Manifesto</label>
      <textarea name="manifesto" class="form-control" rows="3"><?= htmlspecialchars($candidate['manifesto']) ?></textarea>
    </div>
    ...
    <div class="mb-3">
      ...
      <div id="symbolPreview" class="upload-preview">
        <?php if ($candidate['symbol']): ?>
            <img src="../<?= htmlspecialchars($candidate['symbol']) ?>" alt="Current Symbol">
        <?php endif; ?>
      </div>
    </div>
